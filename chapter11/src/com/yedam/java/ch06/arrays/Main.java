...
public class Main {
	...
	public static void main(String[] args) {
		...
		// 방법3
		char[] arr4 = Arrays.copyOfRange(arr1, 1, 3);// 첫번째 인덱스부터 3번째 인덱스 전까지
		System.out.println(Arrays.toString(arr4));

		System.out.println();
		// 배열 항목 비교
		// -equals: 1차 항목의 갑만 비교
		// -deepequals : 1차 항목이 서로다른 배열을 참조할 경우 중첩된 배열의 항목까지 비교
		...
		System.out.println("1차 배열 항목값 비교: " + Arrays.equals(original, cloned1)); // 원본과 동일한 번지를 공유true
		...
		System.out.println("1차 배열 항목값 비교: " + Arrays.equals(original, cloned2)); // 다른 번지를 가지고있어서 false
		System.out.println("중첩 배열 항목값 비교: " + Arrays.deepEquals(original, cloned2));
		System.out.println();

		/*
		 * 배열 항목정렬 -기본 타입(int short..) 또는 String 타입일 경우엔 자동으로 오름차순 정렬 -사용자 정의 클래스 타입일
		 * 경우엔 클래스가 comparable() 구현하고 있어야 정렬이 된다.
		 */
		int[] scores = { 99, 97, 98 };
		Arrays.sort(scores);
		...
		// 배열 항목 검색: 배열항목에서 특정값이 위치한 인덱스를 얻는 것
